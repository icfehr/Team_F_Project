...
### Team_F_Project
### Card Game
# File put together by Team Lead Iain Fehr and Angel Escalera Aguir  
...
2. A **Flask-based web application** that simulates a simple card game with multi-player support and move history tracking. It features a backend to manage game data and an admin panel to view the results.  
   ...
   ***Developed primarily be Angel Escalera Aguir, all base code is his work with only minor modifications made at the end to add a function that receives a ping from the game instance by Iain Fehr.***  
  
   THESE FILES ARE LOCATED WITHIN THE FLASK SERVER BACKEND FOLDER  
   SERVER SETUP IS CONTAINED WITHIN THE README IN THAT DIRECTORY  
