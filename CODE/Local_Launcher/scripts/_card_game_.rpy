init python:
    ...
    def cardgame(enemy_deck, player_deck, shown_backside, reverse, dobelt_number):
        ...
        renpy.hide_screen("basic_functionality_menu")
...
screen card_battle(l_playerdeck, l_enemydeck, shown_cards):
    zorder 13
    imagebutton: # This button serves as the clickable background
        idle Transform("images/cards/card_table.png", xysize=(config.screen_width, config.screen_height))

        action Return("unselect")
    if not selectenemycard < len(l_enemydeck):
        $ selectenemycard = -1

    imagemap:
        at Transform(xysize=(config.screen_width, config.screen_height)) 
        ground "images/cards/card_table.png"
