...
################################################################################################
    ...
    def update_table(x, y, reverse, dobelt_number):
        #"""
        #Updates the game table after a card is placed at coordinates (x, y).
        #It handles card take-overs based on values and game rules.

        #Args:
        #    x (int): The x-coordinate (column) where the card was placed.
        #    y (int): The y-coordinate (row) where the card was placed.
        #    reverse (bool): If True, the take-over logic is reversed (lower value takes over).
        #    dobelt_number (bool): If True, enables a "combo" or "double" rule where matching
        #                          adjacent values can lead to multiple take-overs.
        #"""

        global table_cards

        # Determine the take-over condition based on the 'reverse' rule.
        # If reverse is True, a card with a lower value takes over.
        # Otherwise, a card with a higher value takes over.
        if reverse:
            take_over = lambda a, b : a < b
        else:
            take_over = lambda a, b : a > b

        # Check the card above the newly placed card.
        if not y == 0 and not table_cards[x][y-1] == None and take_over(table_cards[x][y].topvalue, table_cards[x][y-1].bottomvalue):
            # If the new card's top value takes over the card above's bottom value,
            # the card above is flipped to the new card's owner.
            table_cards[x][y-1].playercard = table_cards[x][y].playercard

        # Check the card below the newly placed card.
        if not y == 2 and not table_cards[x][y+1] == None and take_over(table_cards[x][y].bottomvalue, table_cards[x][y+1].topvalue):
            # If the new card's bottom value takes over the card below's top value,
            # the card below is flipped to the new card's owner.
            table_cards[x][y+1].playercard = table_cards[x][y].playercard

        # Check the card to the left of the newly placed card.
        if not x == 0 and not table_cards[x-1][y] == None and take_over(table_cards[x][y].leftvalue, table_cards[x-1][y].rightvalue):
            # If the new card's left value takes over the card to the left's right value,
            # the card to the left is flipped to the new card's owner.
            table_cards[x-1][y].playercard = table_cards[x][y].playercard

        # Check the card to the right of the newly placed card.
        if not x == 2 and not table_cards[x+1][y] == None and take_over(table_cards[x][y].rightvalue, table_cards[x+1][y].leftvalue):
            # If the new card's right value takes over the card to the right's left value,
            # the card to the right is flipped to the new card's owner.
            table_cards[x+1][y].playercard = table_cards[x][y].playercard

        # Handle the "dobelt_number" (combo/double) rule.
        # This rule seems to trigger if the newly placed card has values equal to
        # the facing values of multiple adjacent cards.
        if dobelt_number:
            dobelt_found = [] # List to store coordinates of cards involved in a combo.

            # Check card above for equal values.
            ...
            # Check card to the right for equal values.
            if not x == 2 and not table_cards[x+1][y] == None:
                if table_cards[x][y].rightvalue == table_cards[x+1][y].leftvalue:
                    dobelt_found.append([x+1,y])

            # If the newly placed card created equal-value matches with more than one adjacent card,
            # then all those matched cards are flipped to the new card's owner.
            if len(dobelt_found) > 1:
                for card in dobelt_found:
                    table_cards[card[0]][card[1]].playercard = table_cards[x][y].playercard

################################################################################################
    # Function to transmit the current board state to a server
    # This function is designed to be called when the game state changes
    # and the current board state needs to be sent to a server.

    def transmit_board_state_to_server(server_ip_with_port, current_player_id):
        global table_cards # Access the global table_cards
        # Ensure renpy.log is available. In Ren'Py scripts, this is standard.
        
        serialized_board = []
        for row_idx, row in enumerate(table_cards): # Added enumerate for better error reporting if needed
            serialized_row = []
            for col_idx, card_object in enumerate(row): # Added enumerate
                if card_object is None:
                    serialized_row.append(None)
                else:
                    try:
                        serialized_row.append(card_object.to_dict()) # Uses the to_dict method
                    except AttributeError:
                        # Defensive coding: if a non-Card object or Card without to_dict is on the board
                        renpy.log(f"Warning: Item at table_cards[{row_idx}][{col_idx}] is not a valid Card object or lacks to_dict(). Value: {card_object}")
                        serialized_row.append(str(card_object)) # Fallback to string representation
            serialized_board.append(serialized_row)
        
        try:
            if not server_ip_with_port:
                renpy.log("transmit_board_state_to_server: No server IP provided. Logging board state locally.")
                log_message = "Serialized Board State (Local Log - No Server IP):\n"
                for r_idx, row_data in enumerate(serialized_board):
                    log_message += f"  Row {r_idx}: {row_data}\n"
                renpy.log(log_message)
                return False # Indicate failure to transmit

            # Attempt to send data using the function from webhook.rpy
            # send_board_data itself handles internal exceptions and returns True/False.
            success = send_board_data(server_ip_with_port, current_player_id, serialized_board)

            if not success:
                # send_board_data returned False, indicating the server communication failed.
                # send_board_data should have logged its specific error.
                renpy.log(f"transmit_board_state_to_server: Failed to send board state to {server_ip_with_port} for player {current_player_id} (send_board_data reported failure). Logging board state locally.")
                log_message = "Serialized Board State (Local Log - Server Send Failed):\n"
                for r_idx, row_data in enumerate(serialized_board):
                    log_message += f"  Row {r_idx}: {row_data}\n"
                renpy.log(log_message)
            ...
            # If success is True
            renpy.log(f"transmit_board_state_to_server: Successfully sent board state to {server_ip_with_port} for player {current_player_id}.")
            return True # Transmission successful

        except Exception as e:
            # Catch any other unexpected errors occurring within this function's scope
            renpy.log(f"transmit_board_state_to_server: Exception occurred. Assuming local mode. Error: {e}")
            log_message = "Serialized Board State (Local Log - Exception in transmit function):\n"
            for r_idx, row_data in enumerate(serialized_board):
                log_message += f"  Row {r_idx}: {row_data}\n"
            renpy.log(log_message)
            return False # Indicate failure to transmit
